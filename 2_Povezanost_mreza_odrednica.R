...
shapiro.test(degree(Odrednice_mreza_atributi, mode='out')) # Raspodela nije normalna (p<0.05)
shapiro.test(degree(Podmreza_crvena, mode='in')) # Raspodela nije normalna (p<0.05)
shapiro.test(degree(Podmreza_crvena, mode='out')) # Raspodela je normalna (p=0.13)
shapiro.test(degree(Podmreza_zelena, mode='in')) # Raspodela nije normalna (p<0.05)
shapiro.test(degree(Podmreza_zelena, mode='out')) # Raspodela nije normalna (p<0.05)
shapiro.test(degree(Podmreza_zxuta, mode='in')) # Raspodela je normalna (p=0.08)
shapiro.test(degree(Podmreza_zxuta, mode='out')) # Raspodela je normalna (p=0.27)
# Posto raspodela nije normalna u vecini slucajeva, za sve slucajeve racunacemo medijanu i kvartile

quantile(degree(Odrednice_mreza_atributi, mode='in'))
quantile(degree(Odrednice_mreza_atributi, mode='out'))
quantile(degree(Podmreza_crvena, mode='in'))
quantile(degree(Podmreza_crvena, mode='out'))
quantile(degree(Podmreza_zelena, mode='in'))
quantile(degree(Podmreza_zelena, mode='out'))
quantile(degree(Podmreza_zxuta, mode='in'))
quantile(degree(Podmreza_zxuta, mode='out'))
...
shapiro.test(doseznost_sve.df$Cela_mreza_u) # Raspodela nije normalna (p<0.001)
shapiro.test(doseznost_sve.df$Cela_mreza_iz) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$CK_u)) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$CK_iz)) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$ZeK_u)) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$ZeK_iz)) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$ZuK_u)) # Raspodela nije normalna (p<0.001)
shapiro.test(as.numeric(doseznost_sve.df$ZuK_iz)) # Raspodela nije normalna (p<0.001)

# Stoga ce nam relevantniji biti podaci o medijani i kvartilima:
quantile(doseznost_sve.df$Cela_mreza_u)
quantile(doseznost_sve.df$Cela_mreza_iz)
quantile(na.omit(as.numeric(doseznost_sve.df$CK_u)))
quantile(na.omit(as.numeric(doseznost_sve.df$CK_iz)))
quantile(na.omit(as.numeric(doseznost_sve.df$ZeK_u)))
quantile(na.omit(as.numeric(doseznost_sve.df$ZeK_iz)))
quantile(na.omit(as.numeric(doseznost_sve.df$ZuK_u)))
quantile(na.omit(as.numeric(doseznost_sve.df$ZuK_iz)))
...
shapiro.test(ekscentricnost_sve.df$Cela_mreza_u) # Raspodela nije nirmalna (p<0.001)
shapiro.test(ekscentricnost_sve.df$Cela_mreza_iz) # Raspodela nije normalna (p<0.001)
shapiro.test(ekscentricnost_sve.df$CK_u) # Raspodela je normalnaa (p=0.051)
shapiro.test(ekscentricnost_sve.df$CK_iz) # Raspodela nije normalna (p<0.05)
shapiro.test(ekscentricnost_sve.df$ZeK_u) # Raspodela nije normalna (p<0.01)
shapiro.test(ekscentricnost_sve.df$ZeK_iz) # Raspodela nije normalna (p<0.01)
shapiro.test(ekscentricnost_sve.df$ZuK_u) # za Zutu knjigu raspodela nije normalna (p<0.05)
shapiro.test(ekscentricnost_sve.df$ZuK_iz) # Raspodela nije normalna (p<0.001)

# Posto u vecini slucajeva nema normalne raspodele, racunamo medijanu i kvartile
quantile(ekscentricnost_sve.df$Cela_mreza_u)
quantile(ekscentricnost_sve.df$Cela_mreza_iz)
quantile(na.omit(ekscentricnost_sve.df$CK_u))
quantile(na.omit(ekscentricnost_sve.df$CK_iz))
quantile(na.omit(ekscentricnost_sve.df$ZeK_u))
quantile(na.omit(ekscentricnost_sve.df$ZeK_iz))
quantile(na.omit(ekscentricnost_sve.df$ZuK_u))
quantile(na.omit(ekscentricnost_sve.df$ZuK_iz))
...
summary(Odrednice_mreza_atributi_dz_k) # Dzinovska Komponenta sa 39/45 odrednica
...
Mreza_dz_k_bez_23_2<-delete.vertices(Mreza_dz_k_bez_23, V(Mreza_dz_k_bez_23)$name=="2") # Uklanjamo i cvor 2
is.connected(Mreza_dz_k_bez_23_2) # Vise nije povezana, 
components(Mreza_dz_k_bez_23_2) # ali najveca komponenta je prilicno velika (35/37 cvorova)
Mreza_dz_k_bez_23_2_35<-delete.vertices(Mreza_dz_k_bez_23_2, V(Mreza_dz_k_bez_23_2)$name=="35") # uklanjamo cvor sa 3. vrednoscu CI
components(Mreza_dz_k_bez_23_2_35) # Najveca komponenta je opet velika, 31/36 cvorova
